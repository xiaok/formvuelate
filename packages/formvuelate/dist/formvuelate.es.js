/**
  * formvuelate v3.8.1
  */
import { inject, computed, unref, watch, openBlock, createBlock, resolveDynamicComponent, mergeProps, createCommentVNode, resolveComponent, Fragment, renderList, provide, toRefs, withCtx, renderSlot, toDisplayString, withModifiers, createVNode, isRef, ref } from 'vue';
...
function useParsedSchema(refSchema) {
  ...
  var replaceSubSchemaForms = inject(LOOKUP_PARSE_SUB_SCHEMA_FORMS, null);
  var parsedSchema = computed(function () {
...
function render$3(_ctx, _cache, $props, $setup, $data, $options) {
  return ($setup.schemaCondition)
    ? (openBlock(), createBlock(resolveDynamicComponent($setup.component), mergeProps({ key: 0 }, $setup.binds, {
...
function render$2(_ctx, _cache, $props, $setup, $data, $options) {
  const _component_SchemaField = resolveComponent("SchemaField");
  ...
  return ($setup.rowHasVisibleElements)
    ? (openBlock(), createBlock("div", {
        ...
        (openBlock(true), createBlock(Fragment, null, renderList($props.row, (field) => {
          return (openBlock(), createBlock(_component_SchemaField, mergeProps({
            key: field.model,
            field: field
...
function useParentSchema() {
  var isChildOfWizard = inject(IS_SCHEMA_WIZARD, false);
  var hasParentSchema = inject(PARENT_SCHEMA_EXISTS, false);

  if (!hasParentSchema) {
    provide(PARENT_SCHEMA_EXISTS, true);
  }

  var behaveLikeParentSchema = computed(function () {
...
function useInjectedSchema(props, behaveLikeParentSchema) {
  ...
  var injectedSchema = inject(INJECTED_SCHEMA, false);
  ...
  if (props.nestedSchemaModel) {
    ...
    var path = inject(SCHEMA_MODEL_PATH, '');
...
var forEachSchemaElement = function forEachSchemaElement(schema, fn) {
  ...
  // the whole tree
  var normalizedSchema = normalizeSchema(unref(schema));
...
function useFormModel(props, parsedSchema) {
  var formModel = inject(FORM_MODEL, {});
  var hasParentSchema = inject(PARENT_SCHEMA_EXISTS, false);
...
function render$1(_ctx, _cache, $props, $setup, $data, $options) {
  const _component_SchemaRow = resolveComponent("SchemaRow");

  return (openBlock(), createBlock(resolveDynamicComponent($setup.behaveLikeParentSchema && !$props.useCustomFormWrapper ? 'form' : 'div'), $setup.formBinds, {
    default: withCtx(() => [
      ($setup.behaveLikeParentSchema)
        ? renderSlot(_ctx.$slots, "beforeForm", mergeProps({ key: 0 }, $setup.slotBinds))
        : createCommentVNode("v-if", true),
      (openBlock(true), createBlock(Fragment, null, renderList($setup.parsedSchema, (row, index) => {
        return (openBlock(), createBlock(_component_SchemaRow, {
          key: index,
          row: row,
      ...
      ($props.debug)
        ? (openBlock(), createBlock("pre", _hoisted_1, toDisplayString($setup.parsedSchema), 1 /* TEXT */))
        : createCommentVNode("v-if", true),
      ($setup.behaveLikeParentSchema)
        ? renderSlot(_ctx.$slots, "afterForm", mergeProps({ key: 2 }, $setup.slotBinds))
        : createCommentVNode("v-if", true)
    ]),
    _: 3 /* FORWARDED */
  }, 16 /* FULL_PROPS */))
...
function render(_ctx, _cache, $props, $setup, $data, $options) {
  ...
  return (openBlock(), createBlock("form", {
    onSubmit: _cache[1] || (_cache[1] = withModifiers($event => (_ctx.$emit('submit', $event)), ["prevent"]))
  }, [
    renderSlot(_ctx.$slots, "beforeForm"),
    createVNode(_component_SchemaForm, {
      schema: $setup.currentSchema,
      preventModelCleanupOnSchemaChange: ""
    }, null, 8 /* PROPS */, ["schema"]),
    renderSlot(_ctx.$slots, "afterForm")
  ], 32 /* HYDRATE_EVENTS */))
}
...
function useSchemaForm() {
  ...
  var formModel = isRef(initialFormValue) ? initialFormValue : ref(initialFormValue);
...
export { script$1 as SchemaForm, SchemaFormFactory, script as SchemaWizard, constants, script$1 as default, definePlugin, useSchemaForm };
